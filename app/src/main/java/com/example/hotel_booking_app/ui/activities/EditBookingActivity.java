...
public class EditBookingActivity extends AppCompatActivity {
    ...
    private void loadBooking() {
        ...
        statusTextView.setText("Loading booking...");
    ...
    private void loadCabin(String cabinId) {
        cabinService.getCabinById(cabinId, new SupabaseCallback<Cabin>() {
            ...
            public void onSuccess(Cabin cabin) {
                currentCabin = cabin;
                statusTextView.setText("Booking ready to edit.");
            }

            @Override
            public void onError(String message) {
                statusTextView.setText("Booking ready, but cabin capacity could not be loaded.");
            }
        });
    }
    ...
    private void updateBooking() {
        ...
        try {
            ...
            statusTextView.setText("Guests must be a number.");
            return;
        }
        if (guests <= 0) {
            statusTextView.setText("Guests must be greater than 0.");
            return;
        }
        ...
        bookingService.updateBookingDetails(
                ...
                new SupabaseCallback<Booking>() {
                    @Override
                    public void onSuccess(Booking booking) {
                        Toast.makeText(EditBookingActivity.this, "Booking updated", Toast.LENGTH_SHORT).show();
                        finish();
                    }
