...
public class AdminBookingManagementActivity extends AppCompatActivity {
    ...
    private static final String FILTER_PAID = "Đã thanh toán";
    ...
    private final Set<String> managedCabinIds = new HashSet<>();
    ...
    private void loadBookings() {
        statusTextView.setText("Đang tải đặt phòng bạn quản lý...");
        cabinService.getCabinsForHost(sessionManager.getUserId(), new SupabaseCallback<List<Cabin>>() {
            @Override
            public void onSuccess(List<Cabin> cabins) {
                managedCabinIds.clear();
                cabinById.clear();
                if (cabins != null) {
                    for (Cabin cabin : cabins) {
                        managedCabinIds.add(cabin.getId());
                        cabinById.put(cabin.getId(), cabin);
                    }
                }
                loadBookingsForManagedCabins();
            }
    ...
    private void loadBookingsForManagedCabins() {
            ...
            public void onSuccess(List<Booking> bookings) {
                ...
                if (bookings != null) {
                    for (Booking booking : bookings) {
                        if (managedCabinIds.contains(booking.getCabinId())) {
    ...
    private void renderBookings() {
        ...
        statusTextView.setText("Đang hiển thị " + filtered.size() + "/" + allBookings.size() + " đặt phòng.");
    }
    ...
    private void handlePrimaryAction(Booking booking) {
        if (AppConstants.BOOKING_PENDING.equalsIgnoreCase(booking.getStatus())) {
            updateBooking(booking, AppConstants.BOOKING_CONFIRMED, booking.isPaid(), "Đã xác nhận đặt phòng");
            return;
        }
        if (!booking.isPaid() && !AppConstants.BOOKING_CANCELLED.equalsIgnoreCase(booking.getStatus())) {
            updateBooking(booking, AppConstants.BOOKING_CONFIRMED, true, "Đã đánh dấu đã thanh toán");
            return;
        }
        openBookingDetail(booking);
    }
    ...
    private void cancelBooking(Booking booking) {
        updateBooking(booking, AppConstants.BOOKING_CANCELLED, false, "Đã hủy đặt phòng");
    }
    ...
    private void updateBooking(Booking booking, String status, boolean isPaid, String toast) {
        ...
        bookingService.updateStatus(booking.getId(), status, isPaid, new SupabaseCallback<Booking>() {
            ...
            public void onSuccess(Booking data) {
                Toast.makeText(AdminBookingManagementActivity.this, toast, Toast.LENGTH_SHORT).show();
                loadBookings();
            }
